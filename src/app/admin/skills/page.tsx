...
export default function SkillsPage() {
  ...
  async function addSkill() {
    ...
    // @ts-ignore
    const { data } = await supabase.from("skills").insert(skillToInsert as any).select().single();
  ...
  async function moveSkill(index: number, direction: "up" | "down") {
    ...
    await Promise.all([
      // @ts-ignore
      supabase.from("skills").update({ sort_order: currentSkill.sort_order } as any).eq("id", currentSkill.id),
      // @ts-ignore
      supabase.from("skills").update({ sort_order: targetSkill.sort_order } as any).eq("id", targetSkill.id)
    ]);
  }
